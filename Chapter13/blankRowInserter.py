...
name = 'multi_table.xlsx'# sys.argv[1]
blank_start = 3 # int(sys.argv[2])
blank_length = 2 # int(sys.argv[3])

wb = openpyxl.load_workbook(name, data_only=True)
sheet = wb.active

# Read the data from spreadsheet and get list of row data lists
print('Reading spereadsheet data...')
rows = []
...
for row in range(1, blank_start): # Start at row 1~2
    for cell in range(1, max_column + 1):
        sheet.cell(row=row, column=cell).value = rows[row - 1][cell - 1]


# Write remaining rows after the blank gap
for row in range(blank_start + blank_length, max_row + blank_length + 1): # Star at row 3 + 2 ~ 10 + 2
    for cell in range(1, max_column + 1):
...
print("A copy of the spreadsheet with blanks inserted has been saved as '[blanked-spreadsheet_name]'."
      "It can be found in the same directory as the original.")
