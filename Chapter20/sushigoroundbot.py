...
def getGameRegion():
    ...
    # calcuate the region of the entire game
...
def setupCoordinates():
    ...
    global INGRED_COORDS, PHONE_CORRDS, TOPPING_CORRDS, ORDER_BUTTON_COORDS, RICE1_COORDS, RICE2_COORDS, NORMAL_DELIVERY_BUTTON_COORDS, MAT_COORDS, LEVEL
    ...
    PHONE_CORRDS = (GAME_REGION[0] + 560, GAME_REGION[1] + 360)
    TOPPING_CORRDS = (GAME_REGION[0] + 513, GAME_REGION[1] +269)
...
def navigateStartGameMenu():
    ...
    while True: # loop because it could be the yellow or red Skip button displayed at the moment.
        ...
        if pos is not None:
            break
    pyautogui.click(pos, duration=0.25)
    logging.info('Click on Skip button.')
...
def startServing():
    ...
    while True:
        ...
        if added != {}:
            logging.info('Now orders: %s' % (list(added.values())))
            for k in added:
        ...
        if random.randint(1, 10) == 1 or time.time() - PLATE_CLEARING_FREQ > LAST_PLATE_CLEARRING:
            clickOnPlates()
        ...
        if time.time() - 12 > LAST_GAME_OVER_CHECK:
            ...
            if result == LEVEL_WIN_MESSAGE:
                ...
                if LEVEL <= 7: # click the second continue if the game isn't finished
                    pyautogui.click(pos, duration=0.25)
                    logging.info('Clicked on contunue button')
...
def orderIngredient(ingredient):
    ...
    pyautogui.click(PHONE_CORRDS, duration=0.25)
    ...
    if ingredient == RICE and ORDERING_COMPLETE[RICE] is None:
        ...
        pyautogui.click(TOPPING_CORRDS, duration=0.25)
...
def checkForGameOver():
    ...
    result = pyautogui.locateAllOnScreen(imPath('you_win.png'), region=(GAME_REGION[0] + 188, GAME_REGION[1] + 94, 262, 60))
    if result is not None:
    ...
    if result is not None:
        logging.info('Gmae over. Quitting')
        sys.exit()
